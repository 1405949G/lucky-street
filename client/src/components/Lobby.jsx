...
export default function Lobby({ spectate = false }) {
  ...
  if (kickedPopup) {
    return (
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-[#070b14]/80 ">
  ...
  return (
    <div className="max-w-[860px] mx-auto px-4 pb-10">
      ...
      <div className="flex items-center justify-between pt-3">
        <button onClick={handleLeave} aria-label="Leave room" className="group px-4 py-2 rounded-full bg-[#1e2326] hover:bg-[#1e2326] border border-white/10 text-xs font-bold text-white/70 flex items-center gap-1.5  transition-colors">
          <span className="w-5 h-5 rounded-full bg-[#23272a] group-hover:bg-[#1e2326] flex items-center justify-center text-[11px] leading-none">?</span> Leave
        </button>
        ...
        <button onClick={()=>setShowRules(true)} aria-label="Rules" className="w-9 h-9 rounded-full bg-gradient-to-br from-white/[0.08] to-white/[0.04] hover:from-white/[0.13] hover:to-white/[0.06] border border-white/10 flex items-center justify-center text-[#f3ecd8] font-black text-sm shadow-md ">?</button>
      </div>

      {/* Mobile toggle */}
      {!hasGameState && (
        <div className="mt-4 flex justify-center lg:hidden">
          <div className="inline-flex rounded-full bg-[#1e2326] border border-white/10 p-1 ">
          ...
          {!hasGameState && (
            <div className="rounded-[24px] glass-lantern shadow-xl p-0 text-center relative overflow-hidden">
              ...
              <div className="relative p-6 sm:p-7">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#c9734b]/12 border border-[#c9734b]/18 ">
                ...
                <div className="mt-1 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#23272a] border border-white/10 ">
                ...
                <div className="mt-4 flex justify-center gap-2.5 flex-wrap">
                  ...
                  <span className="px-3.5 py-2.5 rounded-full bg-[#23272a] border border-white/10 text-xs font-black tracking-wide text-white/80 ">{room.slotsText}</span>
                </div>
              </div>
        ...
        <div className="mt-4 flex flex-wrap gap-4">
          ...
          {Array.from({ length: Math.max(0, Math.min(4, room.maxPlayers - room.players.length - room.bots.length)) }).map((_, i) => (
            <div key={`wait-${i}`} className="flex flex-col items-center gap-1.5 opacity-35">
              <div className="w-[68px] h-[68px] rounded-full border-2 border-dashed border-amber-400/25 bg-[#1e2326] flex items-center justify-center ">
          ...
          {isQuestGame && hasGameState ? (
            ...
            <>
              {isQuestGame && (
                <div className="rounded-[20px] glass-library p-4 sm:p-5 relative overflow-hidden">
                  ...
                  {isHost ? (
                    ...
                    <p className={`mt-4 text-center py-3 rounded-full border text-sm font-bold ${canStart ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : 'bg-[#1e2326] border-white/10 text-white/45'}`}>{canStart ? "Ready to start!" : `Need ${room.minPlayers - totalPlayers} more to start`}</p>
                  )}
                </div>
              )}
              ...
              <div className={`rounded-[20px] border p-4 sm:p-5 ${isGameLocked ? "bg-[#1e2326] border-white/10 opacity-60" : "glass-lantern"}`}>
                ...
                {isQuestGame && (
                  <div className="mt-3 flex items-center justify-between px-3 py-2 rounded-xl bg-[#1e2326] border border-white/10">
                    ...
                    <div key={opt.key} className="flex items-center gap-3">
                      ...
                      {opt.type === "toggle" ? (
                        isHost ? (
                          ...
                          <span className={`px-2.5 py-1 rounded-full text-xs font-black border ${room.gameOptions[opt.key] ? "bg-amber-500/15 border-amber-500/20 text-amber-200" : "bg-[#1e2326] border-white/10 text-white/40"}`}>{room.gameOptions[opt.key] ? "On" : "Off"}</span>
                        )
                      ) : opt.type === "slider" ? (
                        isHost ? (
                          <div className="flex-1 flex items-center gap-2">
                            ...
                            <span className="text-xs font-black text-white min-w-[44px] text-right px-2 py-1 rounded-full bg-[#1e2326] border border-white/10">{room.gameOptions[opt.key]}{opt.unit || ""}</span>
                          </div>
                        ) : (
                        ...
                        isHost ? (
                          <select value={room.gameOptions[opt.key]} onChange={e => handleOptionChange(opt.key, e.target.value)} disabled={isGameLocked} className={`flex-1 px-3 py-2.5 rounded-xl border text-xs font-bold ${isGameLocked ? "bg-[#1e2326] border-white/10 text-white/30" : "bg-[#1e2326] border-white/15 text-white focus:border-amber-400/30"}`}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#070b14]/70 " onClick={() => setBotConfirm(null)}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-[340px] rounded-[22px] glass-lantern p-6 text-center shadow-2xl">
            ...
            <div className="mt-5 grid gap-2.5">
              ...
              <button onClick={() => setBotConfirm(null)} className="w-full py-2.5 rounded-full bg-[#1e2326] hover:bg-[#23272a] border border-white/10 text-white font-bold">Close</button>
            </div>
          </div>
        </div>
      )}
