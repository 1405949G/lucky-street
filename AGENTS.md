...
- **Monorepo:** `lucky-street/` is the **full website** (not `reference/Good vs Evil Game/` which is legacy reference for recreating Good vs Evil later).
...
| **Catalog** | Single source: `games/<id>/manifest.js` imported by `server/src/games.js:1`. | `games/good-vs-evil/manifest.js:1` is canonical template |
...
**1 Required File: `games/<id>/manifest.js`** (copy `games/good-vs-evil/manifest.js:1`):
...
export const GAMES = { "good-vs-evil": questOfShadows, "ludo": ludo };
```
...
**Do NOT:** add inside `reference/Good vs Evil Game/`; create workspace sibling; duplicate `GAMES` in client/server.
...
- **Scope AI to one surface per run (critical to avoid retesting all games):**
  ...
  - `visual only — good-vs-evil: edit games/good-vs-evil/client/* only` → lobby/other games untouched
...
- Full website = `lucky-street/`; reference = `reference/Good vs Evil Game/`
